...
struct WeakTemporalLayout: Layout {
    ...
    func closestMatch(values: [Int], inputValue: Int) -> Int? {
        (values.reduce(values[0]) { abs($0 - inputValue) < abs($1 - inputValue) ? $0 : $1 })
    }

    func sizeThatFits(proposal _: ProposedViewSize, subviews _: Subviews, cache: inout CacheData) -> CGSize {
        CGSize(width: cache.maxWidth,
               height: cache.maxHeight)
    }

    func getSubviewMap(key _: any LayoutValueKey.Type, subviews: [LayoutSubview]) -> [Int: [LayoutSubview]] {
    ...
    func placeSubviews(in _: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout CacheData) {
        /*
...
//        var pt = CGPoint(x: bounds.minX, y: bounds.minY)
    ...
    func makeCache(subviews: Subviews) -> CacheData {
        CacheData(maxHeight: computeMaxHeight(subviews: subviews), maxWidth: 600,
                  spaces: computeSpaces(subviews: subviews))
    }

    func updateCache(_: inout CacheData, subviews _: Subviews) {
