...
struct TrackTimeMarkings: View {
    ...
    var body: some View {
        ZStack {
            Rectangle().frame(width: gr.size.width * trackZoomFactor - leftTrackStart - rightTrackEnd, height: 20).position(x: leftTrackStart + (gr.size.width * trackZoomFactor - leftTrackStart - rightTrackEnd) / 2).foregroundColor(Color(NSColor.darkGray))
